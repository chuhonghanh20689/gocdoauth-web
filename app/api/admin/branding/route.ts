...
export async function POST(req:Request){
 ...
 if(ct.includes("application/json")){const body=await req.json();if(body.type==="site_name"){const value=String(body.value||"").trim()||"GÓC ĐỒ AUTH";const {error}=await supabase.from("site_settings").upsert({key:"site_name",value});if(error)return NextResponse.json({error:error.message},{status:500});return NextResponse.json({ok:true,value})}if(body.type==="footer_location"){const value=String(body.value||"").trim()||"Hà Nội, Việt Nam";const {error}=await supabase.from("site_settings").upsert({key:"footer_location",value});if(error)return NextResponse.json({error:error.message},{status:500});return NextResponse.json({ok:true,value})}return NextResponse.json({error:"Dữ liệu không hợp lệ."},{status:400})}
