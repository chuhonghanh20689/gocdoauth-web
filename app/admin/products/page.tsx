...
export default function ProductsAdmin(){
 ...
 return <main className="page"><div className="container">
   ...
   <div className="product-upload" style={{marginTop:18}}>
    ...
    <div className="image-list">
     ...
     {form.images.map((im,i)=><div className="image-card" key={`${im.url}-${i}`}>
      <div className="image-thumb"><img src={im.url} alt={im.alt||form.name}/><button type="button" aria-label={`Xóa ảnh ${i+1}`} onClick={()=>set("images",form.images.filter((_,j)=>j!==i))}>×</button></div>
  ...
  {visible.length?<div className="admin-products">{visible.map(p=><div className="admin-product" key={p.id}>
   <div className="admin-product-image">{p.images?.[0]?.url?<img src={p.images[0].url} alt={p.name}/>:<div className="placeholder">{p.brand}<br/><small>{p.name}</small></div>}</div>
